...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    public ShardedJedisSentinelPool(final GenericObjectPoolConfig poolConfig, List<String> masters, Set<String> sentinels, String password) {
	    ...
	    for (String masterName : masters) {
	    	...
	    	while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
	    		for (String sentinel : sentinels) {
					final HostAndPort node = toHostAndPort(Arrays.asList(sentinel.split(":")));

					logger.debug("Connecting to Sentinel " + node);
					...
					try {
					    ...
					    logger.warn("Cannot connect to sentinel running @ {}. Trying next one.", node);
					}
		    	}
		    	
		    	if (null == master) {
		    		try {
						logger.warn("All sentinels down, cannot determine where is {} master is running... sleeping 1000ms, Will try again.", masterName);
						Thread.sleep(1000);
	    	...
	    	if (!fetched && sentinelRetry >= MAX_RETRY_SENTINEL) {
	    		logger.error("All sentinels down and try {} times, Abort.", MAX_RETRY_SENTINEL);
                    ...
                    public void onMessage(String channel, String message) {
					    ...
					    if (switchMasterMsg.length > 3) {
						    ...
						    if (index >= 0) {
							    ...
							    logger.warn("Ignoring message on +switch-master for master name {}, our monitor master name are [{}]", switchMasterMsg[0], sb.toString());
							}
